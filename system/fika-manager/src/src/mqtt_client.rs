...
fn get_mqtt_options(settings: AWSIoTSettings) -> Result<MqttOptions, mqtt_error::AWSIoTError> {
...
pub async fn async_event_loop_listener(
    ...
    loop {
        match eventloop.poll().await? {
            Event::Incoming(i) => match incoming_event_sender.send(i) {
                Ok(_) => (),
