...
boss_ap_info() {
    ...
    if info=$(fika-manager boss -c $RootFake/$conf/kdaemon.toml get-ap-info) ; then
        echo "user_wallet = \"$(echo $info | jaq -r .user_wallet)\"" >$RootBoss/$conf
        echo "nickname = \"$(echo $info | jaq -r .device_nickname)\"" >>$RootBoss/$conf
        sync;sync
        return 0
    else
        return 127
...
publish_heartbeat() {
    ...
    if msg=$(jaq -rc --null-input \
        --argjson uptime "$uptime" \
        --argjson latency "$latency" \
        --arg systime "$systime" \
        '{"state":{"reported":{"up-time": $uptime, "latency": $latency, "system-time": $systime}},"clientToken": "11111"}'); then
